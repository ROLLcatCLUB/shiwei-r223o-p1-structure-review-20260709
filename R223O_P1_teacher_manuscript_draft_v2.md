...
status=color_manuscript_structure_recovered_draft
preview_only=true
...
本稿仍为 preview-only。它只用于 R223O-P1 教师文稿结构恢复验证，不写入正式备课本，不修改 R97B，不新增正式 UI，不接 runtime、provider/model、prompt 或数据库。
